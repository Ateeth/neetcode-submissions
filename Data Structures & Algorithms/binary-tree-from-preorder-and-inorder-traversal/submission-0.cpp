...
public:
    ...
    TreeNode* buildTree(
        ...
        int inorderIndex = mpp[root->val];
        int numsLeft = inorderIndex - inStart;

        root->left = buildTree(preorder, preStart+1, preStart + numsLeft, inorder, inStart,inorderIndex -1, mpp);
        root->right = buildTree(preorder, preStart + numsLeft + 1, preEnd, inorder, inorderIndex + 1, inEnd, mpp);
